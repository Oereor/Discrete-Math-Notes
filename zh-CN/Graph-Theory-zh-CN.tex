...
\begin{proof}
    ...
    另一方面, 如果存在一个连通子图 $H$ 使得 $G'$ 为其真子图, 则 $H$ 中存在一个顶点 (与 $v$ 连通) 不在 $G'$ 中, 这与 $W$ 的定义矛盾. 因此 $G'$ 是 $G$ 的一个连通块.
\end{proof}
...
\begin{definition}[Strongly Connected]
    Suppose $G=(V,E)$ is a directed graph. Two vertices $u,v\in V$ are \textbf{strongly connected} if there is a path from $u$ to $v$ and a path from $v$ to $u$ in $G$.
\end{definition}

In directed graphs, instead of saying connectedness of two vertices, we say that $v\in V$ is \textbf{reachable} from $u\in V$ if there is a path from $u$ to $v$. Strongly connected vertices are also called \textbf{mutually reachable} vertices. We can also define the concept of \textbf{strongly connected graphs} and \textbf{strongly connected components} in a similar way as we did for undirected graphs, which we shall not do here to avoid redundancy.
